(function() {
	...
	app.directive('medicalNoteDetail', function($compile){
		function link (scope, iElement, iAttrs){
			scope.data = scope.$parent.item;
			scope.showDetail = false;	
			scope.editPath = "#/MedicalNote/edit/"+scope.data.id;
		};
		
		function detailController($scope, $element, medicalNoteFactory){	
			var isDetailLoaded = false;
	...
	controllers.medicalDetailController = function($scope, $routeParams, medicalNoteFactory){
		var id = parseInt($routeParams.id);
		
	    function init(){	
    		medicalNoteFactory.getDetailAsync(id, function(allInfo){
    			$scope.data = allInfo;
    		});		
	    };	
	    init();
